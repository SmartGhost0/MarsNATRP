...
{
    public class MarsNATRPServerConnection
    {
        String clientip;
        ushort clientport;
        ...
        private MarsNATRPServerCore Server
        {
            get;
            set;
        }
        ...
        private void CreateConnectionBridge()
        {
            if (this.Client.Connected)
            {
                this.Proxy = new TcpClient();
                tl = new TcpListener(IPAddress.Parse(this.clientip), this.clientport);
                try
                {
                    tl.Start();
                    Proxy = tl.AcceptTcpClient();
                    tl.Stop();
                    this.Server.WriteLineLog(string.Format("MarsNATRP_Server connect at {0} from {1}...", DateTime.Now, this.Proxy.Client.RemoteEndPoint));
                ...
                catch (Exception ex)
                {
                    ...
                    if (tl.LocalEndpoint != null)
                    {
                        tl.Stop();
                    }
                    else
                    {
                        this.Server.WriteLineLog("An error occurred : " + ex.Message);
                    }
                    this.Close();
                }
                if (tl.LocalEndpoint != null)
        ...
        private void OnClientReceive(IAsyncResult result)
        {
            if (this.Server.IsStarting)
            {
                try
                {
                    ...
                    if (size > 0)
                    {
                        SocketUtils.Send(this.Proxy.Client, _ClientBuffer, 0, size);
        ...
        private void Close()
        {
            try
            {
                if (this.Client != null)
                {
                    this.Server.WriteLineLog(string.Format("MarsNATRP_Server has closed the connection with {1} at {0}.", DateTime.Now, this.Client.RemoteEndPoint));
                    this.Client.Close(3);
                    this.Client = null;
                }
                if (this.Proxy != null)
                {
                    this.Server.WriteLineLog(string.Format("MarsNATRP_Server has closed the connection with {1} at {0}.", DateTime.Now, this.Proxy.Client.RemoteEndPoint));
                    this.Proxy.Close();
                    this.Proxy = null;
                }
            }
